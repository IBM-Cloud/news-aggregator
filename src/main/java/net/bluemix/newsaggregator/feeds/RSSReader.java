...
public class RSSReader {
	...
	public boolean loadFeed() throws Exception {
		if (persistedFeed == null) return false;
		
			URL feedUrl = new URL(persistedFeed.getUrl());
			
			HttpURLConnection httpSource = (HttpURLConnection)feedUrl.openConnection();
	...
	public NewsEntry getNthEntry(int index) {
		...
		title = title.trim();
		if (title.equalsIgnoreCase("")) return null;
		
		String content = getFeedContent(entry);
		if (content == null) return null;
		String abstractContent = cleanHTML(content);
		if (abstractContent == null) return null;
		
		if(abstractContent.length() >= STORY_ABSTRACT_MAXLENGTH) {
			abstractContent = abstractContent.substring(0, STORY_ABSTRACT_MAXLENGTH - 1) + "[...]";
		}
		abstractContent = abstractContent.trim();	
		
		String link = entry.getLink();
		if (link == null) return null;
		if (link.equalsIgnoreCase("")) return null;
		...
		Date date;
		date = new Date();
		newsEntry = new NewsEntry(id.toString(), persistedFeed.getAuthorId(), title, link,
				abstractContent, publicationDate, null, date, date, false, 0,
				NewsEntry.STATE_NEW, null, MODIFIED_BY);
